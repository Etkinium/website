...
import { UtensilsCrossed, MapPin, Clock, Users, Filter, Search, ChevronRight, Star, Heart, Phone } from "lucide-react";
...
const RestaurantCard = ({ index }: { index: number }) => {
  return (
    <div 
    ...
    >
      <div className="relative h-48 md:h-56 bg-gradient-to-br from-gray-800 to-gray-700 overflow-hidden">
        ...
        <div className="absolute top-3 left-3 flex gap-2">
          <span className="bg-accent-amber text-black text-xs font-bold px-3 py-1 rounded-full" data-testid={`restaurant-cuisine-${index}`}>
            —
          </span>
          <span className="bg-green-500 text-white text-xs font-bold px-3 py-1 rounded-full" data-testid={`restaurant-status-${index}`}>
            Açık
          </span>
        </div>
        <button 
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-gray-400 text-sm" data-testid={`restaurant-reviews-${index}`}>(— değerlendirme)</span>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 md:p-5 space-y-3">
        <h3 
          className="text-lg md:text-xl font-bold text-white group-hover:text-accent-amber transition-colors line-clamp-1"
        ...
        <div className="space-y-2 text-sm text-gray-400">
          ...
          <div className="flex items-center gap-2">
            <Users className="w-4 h-4 text-accent-amber flex-shrink-0" />
        ...
        <div className="pt-3 border-t border-gray-700 flex items-center justify-between">
          <div>
            ...
            <p className="text-xl font-bold text-accent-amber" data-testid={`restaurant-price-${index}`}>—— ₺</p>
          </div>
          <Button 
            size="sm"
      ...
      <main className="pt-20 pb-16">
        ...
        <div className="container mx-auto px-4">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
            <div>
              <h1 className="text-3xl md:text-4xl font-bold flex items-center gap-3">
                <UtensilsCrossed className="w-8 h-8 text-accent-amber" />
                Restoranlar
              </h1>
              <p className="text-gray-400 mt-1">En seçkin restoranlardan kolayca rezervasyon yapın</p>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {[...Array(8)].map((_, index) => (
              <RestaurantCard key={index} index={index} />
            ))}
          </div>
